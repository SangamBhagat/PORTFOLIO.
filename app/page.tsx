  ...
  return <>
    ...
    <main>
      ...
      <section id="contact" className="contact"><video className="contact-video" autoPlay loop muted playsInline preload="metadata" aria-hidden="true"><source src="/connect-background.mp4" type="video/mp4" /></video><div className="contact-overlay" aria-hidden="true" /><p className="label">Connect</p><h2>LET&apos;S CONNECT.</h2><div className="connect-grid">
        ...
        <a className="connect-card" href="https://www.instagram.com/mann.utf/" target="_blank" rel="noreferrer" aria-label="Visit Bheem's Instagram"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3.5" y="3.5" width="17" height="17" rx="4" /><circle cx="12" cy="12" r="4" /><circle cx="17.5" cy="6.8" r="1" fill="currentColor" stroke="none" /></svg><span>Instagram</span></a>
        <a className="connect-card" href="https://x.com/MannuRajpu57574" target="_blank" rel="noreferrer" aria-label="Visit Bheem on X"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 4 19 20M19 4 5 20" /></svg><span>X / Twitter</span></a>
      </div></section>
      <section className="quote-wall" aria-label="Batman quotes"><p className="label">Words from the shadows</p><div className="quote-window"><div className="quote-track">{[...batmanQuotes, ...batmanQuotes].map((quote, index) => <p className="quote-card" key={`${quote}-${index}`}>“{quote}”</p>)}</div></div></section>
    </main>
  </>;
}
